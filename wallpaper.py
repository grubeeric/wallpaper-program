#!/usr/bin/python2.7
#change the wallpaper every day to the top post of the day on /r/wallpapers
#Eric Grube

import re, praw, requests, os, glob, sys, ctypes
...
def setImage(localFileName):
    print "Hi There I am executing!"
    filePath = os.path.abspath(localFileName)
    print filePath
...
r = praw.Reddit(user_agent='This guy goes to a subreddit and grabs the top post. Designed to go to /r/wallpapers and get the top pic for a wallpaper of the day') 
submissions = r.get_subreddit(targetSubreddit).get_hot(limit=1)
